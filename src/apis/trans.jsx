...
import { msAuth, invalidateMsToken } from "../libs/auth";
...
const genGoogle = async ({ texts, from, to, url, key }) => {
...
const genGoogle2 = async ({ texts, from, to, url, key }) => {
...
const genMicrosoft = async ({ texts, from, to }) => {
  if (!texts || texts.length === 0) return { url: "", body: null, headers: {} };
  ...
  const params = queryString.stringify(query, { skipEmptyString: true });
  
  const token = await msAuth();
  if (!token) throw new Error("Microsoft Auth Token is invalid");
...
const genAzureAI = async ({ texts, from, to, key, url, region }) => {
  const params = queryString.stringify({
    from,
    to,
  });
...
const genDeepl = async ({ texts, from, to, url, key }) => {
  const body = {
    text: texts,
    target_lang: to,
    source_lang: from,
...
const genDeeplX = async ({ texts, from, to, url, key }) => {
  const body = {
    text: texts.join(" "),
    target_lang: to,
    source_lang: from,
...
const genNiuTrans = async ({ texts, from, to, url, key, dictNo, memoryNo }) => {
  const body = {
    from,
    to,
    apikey: key,
...
const genTencent = async ({ texts, from, to }) => {
  const body = {
    header: {
      fn: "auto_translation",
      client_key:
...
const genVolcengine = async ({ texts, from, to }) => {
  const body = {
...
const genCloudflareAI = async ({ texts, from, to, url, key }) => {
  const body = {
    text: texts.join(" "),
    source_lang: from,
    target_lang: to,
...
const genCustom = async ({ texts, fromLang, toLang, url, key, useBatchFetch }) => {
...
export const genTransReq = async ({ reqHook, ...args }) => {
  // 如果没有提供 texts 只有 events，自动提取文本数组供机器翻译引擎使用
  if (!args.texts && args.events) {
    args.texts = args.events.map((e) =>
  ...
  const {
    ...
    userMsg = null,
    method = "POST",
  } = await genReqFuncs[apiType](args);
...
export const handleTranslate = async (
  ...
  try {
    ...
    // The original `if (!result?.length)` check is implicitly removed here
    // as the new structure returns `result` directly from the try block.
    return result;
  } catch (err) {
    // 如果是微软翻译报 401，立即让当前 Token 失效
    if (apiType === OPT_TRANS_MICROSOFT && (
      err.message?.includes('"status":401') || 
      err.message?.includes('401') ||
      err.message?.includes('Unauthorized')
    )) {
       await invalidateMsToken();
       // 失效后重试一次
       try {
         const newToken = await msAuth();
         if (newToken) {
           const [retryInput, retryInit] = await genTransReq({
             texts, from, to, fromLang, toLang, langMap, docInfo, glossary, ...apiSetting,
           });
           const retryRes = await fetchData(retryInput, retryInit, {
             useCache: false, usePool, fetchInterval, fetchLimit, httpTimeout,
           });
           if (retryRes) {
             return parseTransRes(retryRes, { texts, from, to, fromLang, toLang, langMap, userMsg, ...apiSetting });
           }
         }
       } catch (retryErr) {
         // 重试也失败了，抛出原始错误
...
export const handleSubtitle = async ({ events, from, to, apiSetting }) => {
  ...
  switch (apiType) {
    ...
    case OPT_TRANS_CUSTOMIZE:
      return res;
    default: {
      const machineTexts = events.map((e) =>
        (e.segs || []).map((s) => s.utf8).join("")
      );
      const result = await parseTransRes(res, { apiType, texts: machineTexts, from, to });
