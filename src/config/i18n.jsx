...
export const I18N = {
  import_subtitle: {
    zh: `导入 TheBoringEnglish`,
    en: `Import TheBoringEnglish`,
    zh_TW: `匯入 TheBoringEnglish`,
    ja: `TheBoringEnglish に導入`,
    ko: `TheBoringEnglish 로 가져오기`,
  },
  app_name: {
    zh: `TheBoringEnglish`,
    en: `TheBoringEnglish`,
  ...
  sync_warn: {
    zh: `涉及隐私数据的同步请谨慎选择第三方同步服务，建议自行搭建 lingoflow-worker 或 WebDAV 服务。`,
    en: `When synchronizing data that involves privacy, please be cautious about choosing third-party sync services. It is recommended to set up your own sync service using lingoflow-worker or WebDAV.`,
    zh_TW: `同步涉及隱私資料時，請謹慎選擇第三方同步服務；建議自建 lingoflow-worker 或 WebDAV 服務。`,
    ja: `プライバシーに関わるデータを同期する場合、サードパーティの同期サービスは慎重に選択してください。lingoflow-worker や WebDAV サービスを自己ホスティングすることをお勧めします。`,
    ko: `개인정보가 포함된 데이터를 동기화할 경우, 타사 동기화 서비스 선택에 신중을 기하십시오. 자체 lingoflow-worker 또는 WebDAV 서비스를 구축하는 것을 권장합니다.`,
  },
  sync_warn_2: {
  ...
  about_sync_api: {
    zh: `自建lingoflow-worker数据同步服务`,
    en: `Self-hosting a TheBoringEnglish-worker data sync service`,
    zh_TW: `自建 lingoflow-worker 資料同步服務`,
  ...
  fixer_function_helper: {
    zh: `1、br是将<br>换行替换成<p "lingoflow-p">。2、bn是将\\n换行替换成<p "lingoflow-p">。3、brToDiv和bnToDiv是替换成<div class="lingoflow-p">。`,
    en: `1. br replaces <br> line breaks with <p "lingoflow-p">. 2. bn replaces \\n newline with <p "lingoflow-p">. 3. brToDiv and bnToDiv are replaced with <div class="lingoflow-p">.`,
    zh_TW: `1. br 會將 <br> 換行替換為 <p "lingoflow-p">。2. bn 會將 \\n 換行替換為 <p "lingoflow-p">。3. brToDiv 與 bnToDiv 會替換為 <div class="lingoflow-p">。`,
    ja: `1. br は <br> 改行を <p "lingoflow-p"> に置き換えます。 2. bn は \\n 改行を <p "lingoflow-p"> に置き換えます。 3. brToDiv と bnToDiv は <div class="lingoflow-p"> に置き換えます。`,
    ko: `1. br은 <br> 줄바꿈을 <p "lingoflow-p">로 대체합니다. 2. bn은 \\n 줄바꿈을 <p "lingoflow-p">로 대체합니다. 3. brToDiv 및 bnToDiv는 <div class="lingoflow-p">로 대체됩니다.`,
  },
  import: {
    zh: `导入`,
    en: `Import`,
  ...
  error_sync_setting: {
    zh: `您的同步类型必须为“LINGOFLOW-Worker”，且需填写完整`,
    en: `Your sync type must be "LINGOFLOW-Worker" and must be filled in completely`,
    zh_TW: `您的同步型態必須為「LINGOFLOW-Worker」，且需填寫完整。`,
    ja: `同期タイプは「LINGOFLOW-Worker」である必要があり、すべて入力する必要があります。`,
    ko: `동기화 유형은 "LINGOFLOW-Worker"여야 하며, 모든 항목을 빠짐없이 입력해야 합니다.`,
  },
  click_test: {
    zh: `点击测试`,
    en: `Click Test`,
